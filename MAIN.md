...
| 1-6 | 雙欄對等 | col | 將兩個產品卡片放入 row 中，讓它們平均分配為兩個等寬區塊。直接在 col 上設定背景色，觀察與下一關的差異。 |
| 1-7 | 多包一層 | col | 依照1-6的範例多包一層。因為 col 使用 padding 做為區塊之間的間距，所以要在 `<div class="col"></div>` 內多包一層區塊，例如 `<div class="bg-primary">內容</div>`。之後的 1-8~1-12 都要多包一層下層 `<div class="bg-primary">內容</div>` |
| 1-8 | 三欄佈局 | col | 將三個服務圖標放入 row 中，平均分成三個等寬區塊。 |
| 1-9 | 五等份挑戰 | col | 將五個步驟說明放入 row 中，讓它們等寬並排。 |
| 1-10 | 多筆資料 | col-4 | 若要呈現第二列以後的資料，可以使用 col-* 固定區塊寬度 |
| 1-11 | 左窄右寬 | col-4, col-8 | 創建一個左側較窄、右側較寬的佈局：左側佔 4 欄，右側佔 8 欄。 |
| 1-12 | 左寬右窄 | col-10, col-2 | 創建管理者後台常見的佈局：主要數據表格佔 10 欄，操作按鈕區佔 2 欄。 |
...
| 2-2 | 頂部留白 | pt-5 | 調整標題的上內邊距 (Padding Top) 為級別 5 (最大間距)。 |
...
| 2-6 | 間距微調 | mt-3, px-4 | 調整卡片的上邊距 (mt) 為 3，左右內邊距 (px) 為 4，實現視覺微調。 |
